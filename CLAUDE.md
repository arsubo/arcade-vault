...
- `/spec-impl-game` es la variante de `/spec-impl` para specs que dan de alta un juego: sigue sus mismas fases al pie de la letra (las lee de `.claude/skills/spec-impl/SKILL.md`, no las duplica) y al terminar encadena automáticamente `skin-designer` y luego `mobile-porter` sobre el `game-id` recién implementado, con un commit de checkpoint entre cada etapa. Ver `.claude/skills/spec-impl-game/SKILL.md`.
...
- `mobile-porter` (`.claude/agents/mobile-porter.md`) también escribe código directamente, como `skin-designer`. Repite sobre **un solo juego por corrida** el trabajo que `specs/10-controles-tactiles.md` hizo una vez sobre los 4 juegos originales: refactoriza el motor a una sola ruta de input, implementa `setVirtualKey`, publica el `inputRef` en el wrapper, escribe el binding de los 6 controles del pad en `lib/touch-controls.ts`, y ajusta la proporción del marco CRT en móvil si el tablero no es 4:3. `TouchControls.tsx` y `JugarClient.tsx` son data-driven y no los toca. Lleva su progreso en `references/games-with-touch.md`. No lo encadenan `/add-game` ni `/spec-impl` — hay que invocarlo a mano indicándole el `game-id` a portar —, salvo la excepción explícita de `/spec-impl-game`, que sí lo encadena automáticamente (después de `skin-designer`, con el `game-id` ya confirmado por el usuario). Sin `game-id` e invocado a mano, reporta qué juegos ya tienen pad táctil y pide cuál seguir.
