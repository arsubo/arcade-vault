...
- Layout móvil del resto del sitio: `Nav` (en la captura "ARCADE VAULT" queda pisado por "Iniciar Sesión" — bug real, confirmado, pero fuera de este spec), home, `/biblioteca`, `/salon`, `/games/[id]`, `/acerca-de`, `/auth`. Durante la implementación el usuario pidió puntualmente ocultar/remover por completo el `Nav` en `/jugar` cuando el pad táctil está visible (móvil); sigue fuera de alcance de este spec — queda anotado como candidato concreto para el spec de layout móvil del sitio.
...
9. **Alto de la pantalla.** Acotar el `.crt-screen` en móvil para que HUD + CRT + pad entren sin scroll: en `@media (pointer: coarse)`, `width: min(100%, calc(var(--crt-max-h) * var(--crt-ratio-w) / var(--crt-ratio-h))); margin-inline: auto;` con `--crt-max-h` distinto en portrait y en landscape. `--crt-ratio-w`/`--crt-ratio-h` (default `4`/`3`) reemplazan el literal `aspect-ratio: 4/3` de `.crt-screen` para que ambas reglas compartan la misma proporción. **Excepción para Tetris:** su tablero es naturalmente vertical (300×600 = 1:2) y en 4:3 quedaba angosto con barras negras a los costados; el usuario pidió puntualmente, viendo una captura de referencia, que en móvil el marco de Tetris respete esa proporción vertical. Solo bajo `@media (pointer: coarse)` y solo para este juego (vía `.av-player[data-game="tetris"] .crt-screen`, con `data-game={game.id}` agregado en `JugarClient`), `--crt-ratio-w: 1; --crt-ratio-h: 2;`. Los otros 3 juegos y el escritorio siguen en 4:3; el letterboxing interno de cada motor, el tamaño de los botones del pad y la ubicación de PAUSA/REGRESAR en el HUD no se tocan.
...
- **Sí:** `.touch-pad` con `margin-top: 34px` (antes 18px) — el usuario pidió más separación entre el CRT y la flecha superior del d-pad.
- **Sí:** dentro del menú `⋮`, el botón antes rotulado `SALIR` ahora dice `REGRESAR` — mismo destino (`/games/[id]`), mismo lugar; el `SALIR` del HUD de escritorio (fuera del `⋮`, en `.hud-actions`) no cambia.
- **Sí:** excepción de proporción de pantalla para Tetris en móvil (`--crt-ratio-w/h: 1/2` vía `.av-player[data-game="tetris"] .crt-screen`, solo bajo `pointer: coarse`) — el usuario mostró una captura de referencia con el marco angosto y vertical, calcado a la proporción real del tablero (300×600). Los otros 3 juegos y el escritorio siguen en 4:3.
