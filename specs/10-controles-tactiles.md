...
7. **CSS del pad.** En `app/globals.css`: `.touch-pad` con `display: none` por defecto y `display: grid` bajo `@media (pointer: coarse)`; `touch-action: none`, `user-select: none`, `-webkit-tap-highlight-color: transparent`, `overscroll-behavior: contain`; área táctil mínima de 56×56 px por control; estilo coherente con `.btn` y con las variables `--skin-*` ya existentes, para que el pad cambie de color junto con la skin. Sistema funcional: el juego se controla con el dedo en un teléfono real y sigue igual en escritorio.
